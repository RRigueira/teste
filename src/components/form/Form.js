import { Wrapper, Container, Field, Message, Toggle } from "./Form.styles"
...
const Form = ({ lang }) => {
  ...
  const sendMessage = () => {
    ...
    data2.append("yourmessage", contact.message)
    data2.append("yoursubject", "your-subject")
    data2.append("youremail", contact.email)
    data2.append("yourname", contact.name)
    var config = {
      method: "post",
      url: `http://wp-gatsby.dev.others.pt/wp-json/contact-form-7/v1/contact-forms/${data.id}/feedback`,
      headers: {
    ...
    <Wrapper id="form" onSubmit={handleSubmit(onSubmit)}>
      ...
      <Container>
        <Field>
          ...
          />
          {errors.name && <p>Precisamos do teu nome.</p>}
        </Field>
        ...
        <Field isFull={true}>
          ...
          {errors.message && errors.message.type === "required" && (
            <p>Precisamos que escrevas uma mensagem.</p>
          )}
